...
def find_all_time_highs_and_drawdowns(df, threshold=5):
    """Find all-time highs and subsequent drawdowns > threshold%"""
    df = df.copy()
    ...
    # Find drawdown periods > threshold%
    drawdown_periods = []
    current_ath = None
    ...
    for idx, row in df.iterrows():
        if row['close'] == row['cummax']:  # New all-time high
            if current_ath is not None and current_ath_idx is not None:
                ...
                if min_dd < -threshold:  # More than threshold% drawdown
...
def plot_cycles_with_annotations(df, ath_points, drawdown_periods, tqqq_df=None, tqqq_periods=None, threshold=5):
    ...
    ax1.set_title(f'QQQ Price Cycles: All-Time Highs and >{threshold}% Drawdowns', fontsize=14, fontweight='bold')
    ...
    if tqqq_df is not None and tqqq_periods is not None:
        ...
        ax2.set_title(f'TQQQ Performance During QQQ Cycles (>{threshold}% threshold)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f'../qqq_tqqq_cycles_{threshold}pct.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"QQQ and TQQQ cycles analysis plot saved as 'qqq_tqqq_cycles_{threshold}pct.png'")

def print_cycle_summary(drawdown_periods, tqqq_periods=None, threshold=5):
    """Print detailed summary of all cycles with TQQQ data"""
    print("\n" + "="*100)
    print(f"QQQ & TQQQ CYCLE ANALYSIS: All-Time Highs and >{threshold}% Drawdowns")
    print("="*100)
...
def main():
    ...
    print(f"Loading QQQ and TQQQ data and analyzing price cycles with {threshold}% threshold...")
    ...
    try:
        ...
        # Find cycles
        ath_points, drawdown_periods = find_all_time_highs_and_drawdowns(qqq_df, threshold)
        
        print(f"Found {len(ath_points)} all-time high points")
        print(f"Found {len(drawdown_periods)} drawdown periods > {threshold}%")
        ...
        # Print summary
        print_cycle_summary(drawdown_periods, tqqq_periods, threshold)
        
        # Create visualization
        plot_cycles_with_annotations(qqq_df, ath_points, drawdown_periods, tqqq_df, tqqq_periods, threshold)
